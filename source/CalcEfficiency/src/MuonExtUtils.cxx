...
#include "CalcEfficiency/easy_logger.h"
...
pair< double, double > MuonExtUtils::extTrack( const xAOD::TrackParticle* trk ) {
  ...
  if( fabs(trk->eta()) < 1.05 ) {
    ...
    if( !extRPC ) {
      ...
      if( !extTGC ) {
        LOGD << "MuonExtUtils: both extrapolated track parameters on RPC and TGC is null!" << endl;
        return make_pair( ExtEta, ExtPhi );
      }
...
//  double pt     = feature->pt()*1000.;      //unused
  ...
  //int middle = (feature->sAddress() == -1) ? xAOD::L2MuonParameters::Chamber::EndcapMiddle : xAOD::L2MuonParameters::Chamber::BarrelMiddle; unused variable
...
//*/
  ...
  double extr_eta;
  double extr_phi;
//  double extr_pt;  unused-but-set variable
...
//     double extr_q_over_p = muonPerigee -> parameters()[Trk::qOverP];  //unused
     extr_eta      = -log(tan(extr_theta / 2.));
//     int    extr_q        = (extr_q_over_p > 0 ? 1 : -1);      //unused
